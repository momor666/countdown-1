...
subject to {
	...
	forall(i in R){
		res[i] >= 0;
		/* FIXME constraint on result doesn't work for target = 8 and values = [2, 3] */
		res[i] <= pow(prod(j in N) values[j],4); /* an arbitrary upper bound for intermediate result */
	}
	...
	forall(i in N){
		((op[i] == 1) => (res[i] + values[index[i]] == res[i + 1])) &&
			...
			((op[i] == 6) => ((pow(res[i], values[index[i]]) == res[i + 1]) && values[index[i]] <= 10));
	}
