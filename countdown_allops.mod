...
subject to {
    ...
    /* each operator is used at most once except the absorption */
	count(all(i in N) op[i], 1) <= 1;
	count(all(i in N) op[i], 2) <= 1;
	count(all(i in N) op[i], 3) <= 1;
	count(all(i in N) op[i], 4) <= 1;
